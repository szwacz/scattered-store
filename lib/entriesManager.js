...
module.exports.create = function (storageDir) {
    ...
    function get(key) {
        ...
        .then(function (buf) {
            ...
            if (err.code === 'ENOENT') {
                // if there is no file, treat it as no value for given key
    ...
    function del(key) {
        var deferred = Q.defer();
        var filePath = transformKeyToFilePath(key);
        jetpack.removeAsync(filePath, { safe: true })
        .then(function () {
            deferred.resolve();
        }, function (err) {
            if (err.code === 'ENOENT') {
                // if already there is no file (so key never existed),
                // this is no big deal for us, and no need to cause an error
                deferred.resolve();
            } else {
                deferred.reject(err);
            }
