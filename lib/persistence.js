...
const encodeForStorage = (key, value) => {
  let type;
  let data;
...
module.exports.create = (storageDirPath, callback) => {
  // ----------------------------------------------
  // Initialization
  ...
  const transformKeyToFilePath = (key) => {
  ...
  const get = function (key) {
    ...
    const filePath = transformKeyToFilePath(key);
    storageDir.readAsync(filePath, 'buffer')
    .then((buf) => {
      if (buf) {
